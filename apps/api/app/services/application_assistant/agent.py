"""Autonomous application preparation and failure analysis agent for CareerOS."""
...
KV_AGENT_RUNS = "application_agent_runs"

AGENT_DEBUG_SYSTEM = """You are an intelligent application automation assistant inside CareerOS.
...
AGENT_SYSTEM = """You are an autonomous application assistant inside CareerOS.
...
QWEN_AGENT_SYSTEM = AGENT_SYSTEM


def _get_agent_runs(db: Session) -> dict[str, Any]:
    return get_kv(db, KV_AGENT_RUNS) or get_kv(db, "qwen_agent_runs") or {}
...
def reconcile_stale_prep_state(
    ...
    *,
    ...
    prep_running = task_status(f"prep_{app_id}") == "running" or task_status(f"qwen_prep_{app_id}") == "running"
...
async def execute_application_open_review(
    ...
    *,
    ...
    if is_prep_task_running(app_id):
        return {
            "success": False,
            "error": "The assistant is still preparing this application — wait for prep to finish, then use the browser window it opened.",
            "status": "preparing",
        }
...
def get_review_session_status(db: Session, app_id: str) -> dict[str, Any]:
    ...
    if draft.get("status") == "blocked":
        ...
        return with_quick_apply({
            "status": "failed",
            "browserOpen": False,
            "message": err_msg or "Application is blocked — try asking the assistant to resume.",
            "jobUrl": draft.get("jobUrl", ""),
        })
...
def _rule_based_prep_failure_summary(bundle: dict[str, Any], prep_result: dict[str, Any]) -> str | None:
    ...
    if "timeout" in combined:
        return (
            "Prep timed out while Playwright or the LLM was still working. "
            "Keep Ollama/LLM running, then click Start preparation again. "
    ...
    if status == "ready_to_prepare" and not prep_result.get("success"):
        return (
            ...
            "Click Start preparation again and watch the live panel for browser + mapping steps."
        )
    if stopped:
...
async def _autonomous_prepare_app(app_id: str) -> None:
    ...
    with session_scope() as db:
        ...
        _log_agent(
            ...
            model=model,
            success=True,
            summary=f"Starting prep: {draft.get('companyName')} — {draft.get('roleTitle')}",
            metadata={"applicationId": app_id, "jobUrl": draft.get("jobUrl", "")},
        )
...
async def start_autonomous_prepare_for_job(job_id: str) -> dict[str, Any]:
    """Create application draft (if needed) and start prep in background."""
    with session_scope() as db:
...
async def start_autonomous_prepare(app_id: str) -> dict[str, Any]:
    """Kick off background prep for an existing application."""
    try:
        with session_scope() as db:
            draft = get_application_draft(db, app_id)
            if not draft:
            ...
            prep_task_state = task_status(f"prep_{app_id}")
            if prep_task_state != "running":
    ...
    await run_in_background(f"prep_{app_id}", _start())
...
def build_chat_context(db: Session, context: dict[str, Any]) -> str:
    """Enrich chat with latest agent run and diagnostics."""
    parts: list[str] = []
    ...
    if app_id:
        ...
        if run:
            parts.append(f"Latest agent run for {app_id}: {json.dumps(run)[:1500]}")
        if draft:
            parts.append(
        ...
        if run and run.get("analysis"):
            parts.append(f"Prior analysis: {run['analysis'][:800]}")
