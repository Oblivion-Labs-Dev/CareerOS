"""Workday ATS provider adapter — placeholder, not yet supported."""
...
class WorkdayAdapter(ProviderAdapter):
    ...
    def detect(self, url: str, page_content: str = "") -> ProviderDetection:
        ...
        if re.search(r"myworkdayjobs\.com|/recruiting/", text):
    ...
    async def discover_jobs(self, page: Any, options: dict[str, Any] | None = None) -> list[JobListing]:
        raise NotImplementedError("Workday adapter is not yet supported")
    ...
    async def detect_blocker(self, page: Any) -> dict[str, Any] | None:
        return {"type": "unsupported_provider", "message": "Workday is not yet supported"}
