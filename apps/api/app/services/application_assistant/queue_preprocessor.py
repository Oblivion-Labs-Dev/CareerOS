...
# Maximum number of jobs allowed to sit in QUEUED. This caps how many *new*
# postings the preprocessor adds; rows already queued are never removed to get
# under it. Set CAREEROS_MAX_QUEUE_SIZE=0 for the previous unbounded behaviour.
MAX_QUEUE_SIZE = int(os.environ.get("CAREEROS_MAX_QUEUE_SIZE", "50"))
...
class QueuePreprocessor:
    ...
    def _enqueue_scored_jobs(self) -> int:
        ...
        headroom = (MAX_QUEUE_SIZE - queued_now) if MAX_QUEUE_SIZE > 0 else None
        if headroom is not None and headroom <= 0:
            # At capacity. Existing rows stay untouched — the cap throttles
            # intake, it does not evict work that is already prepared.
        ...
        precomputed = {str(j["id"]): j["mistralMatch"] for j in candidates}
        ...
        with session_scope() as db:
            for r in ranked:
                ...
                is_dup, _existing = is_duplicate_application(db, company, title, url)
                if is_dup:
                    deduped += 1
                    continue
                ...
                enqueued += 1
                if headroom is not None and enqueued >= headroom:
                    logger.info(
                        "Queue preprocessor reached the queue cap (%d); pausing intake",
                        MAX_QUEUE_SIZE,
                    )
                    break
