...
export function PipelineKanban() {
  ...
  return <div className={styles.workspace}>
    <div className={styles.toolbar}><div><span className={styles.eyebrow}>APPLICATIONS / THE BIG PICTURE</span><h1>See your search take shape.</h1><p>Follow each opportunity from application to outcome.</p></div><div className={styles.actions}><input ref={fileInputRef} type="file" accept=".csv,text/csv" hidden onChange={handleFileChange} /><button onClick={handleImportClick} disabled={importBusy}>{importBusy ? 'Importing…' : 'Import CSV'}</button><button onClick={() => void downloadApplicationsCsv().catch(err => setImportMessage(err instanceof Error ? err.message : 'Export failed'))}>Export CSV ↗</button><button onClick={() => void load()}>Refresh ↻</button></div></div>
